...
require('lib/fpdf.php');
    ...
    class PDF extends FPDF
    {
      ...
      function Header()
      {
        $info = DB::select("SELECT nombre,apellido ,nombre_largo
                    FROM grupo_empresa, usuario
                    WHERE id_grupo_empresa=:id_grupoem AND consultor_tis=id_usuario",['id_grupoem'=>$this->id_grupoem]);
        ...
        $this->Cell(0,10,utf8_decode("Consultor TIS: ".$info[0]->nombre." ".$info[0]->apellido),0,'L');
        $this->Ln(4);
        $this->Cell(0,10,utf8_decode("Grupo Empresa: ".$info[0]->nombre_largo),0,'L');
        $this->Ln(4);
        $fech=date("d-m-Y");
    ...
    $pdf->Ln(10);
    $res= DB::select("SELECT ep.descripcion,ep.fecha_inicio,ep.fecha_fin,ep.pago_establecido,ep.pago_recibido ,ep.id_entrega_producto
                      FROM entrega_producto ep
                      WHERE ep.grupo_empresa=:idGrupo",['idGrupo'=>$idGrupo]);    
...
/*---------------------FIN PRIMERA INFO -------------------*/
  ...
  $pdf->Ln(10);
  $res = DB::select("SELECT ep.descripcion,ep.id_entrega_producto,ep.enlace_producto,ep.fecha_inicio,ep.fecha_fin,ep.pago_recibido,ep.pago_establecido
               FROM entrega_producto ep
               WHERE ep.grupo_empresa=:id_grupoem",['id_grupoem'=>$idGrupo]);
    /*COLORES*/
    $pdf->SetFillColor(255,255,0);
    $pdf->SetTextColor(90,90,90);
    $pdf->SetDrawColor(49,126,172);
  ...
  $pdf->Ln(10);
  $res = DB::select("SELECT age.descripcion,age.fecha_inicio,age.fecha_fin,age.porcentaje_completado
                          FROM actividad_grupo_empresa age, entrega_producto e
                          WHERE age.entrega_producto=e.id_entrega_producto AND e.grupo_empresa=id_grupoem",['id_grupoem'=>$idGrupo]);
  $pdf->SetFillColor(255,255,0);
  $pdf->SetTextColor(90,90,90);
  $pdf->SetDrawColor(49,126,172);
  ...
  /*---------------------cuarta INFO----------------------------*/
    ...
    $pdf->Ln(10);
    $res = DB::select("SELECT t.descripcion,t.fecha_inicio,t.fecha_fin,u.nombre,u.apellido,t.porcentaje_completado,t.resultado_obtenido
                            FROM tarea t,usuario u,actividad_grupo_empresa age, entrega_producto e
                            WHERE t.actividad=age.id_actividad and t.responsable=u.id_usuario AND age.entrega_producto=e.id_entrega_producto AND e.grupo_empresa=id_grupoem",['id_grupoem'=>$idGrupo]);    
    $pdf->SetFillColor(255,255,0);
    $pdf->SetTextColor(90,90,90);
    $pdf->SetDrawColor(49,126,172);
  ...
  $pdf->Ln(10);
  $res =DB::select("SELECT id_usuario,nombre,apellido,i.grupo_empresa,u.email,u.telefono,i.codigo_sis,c.nombre_carrera
                         from usuario u, integrante i,carrera c
                         where u.habilitado=1 AND i.usuario=u.id_usuario AND i.grupo_empresa=:id_grupoem AND c.id_carrera=i.carrera",['id_grupoem'=>$idGrupo]);
