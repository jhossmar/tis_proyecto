...
if(isset($_GET['entreprod'])){
  $id_act=$_GET['entreprod'];
}else{
  header('Location:cronograma_grupo_integra.php');
}
$titulo="Tareas de la actividad";
...
$consulta_no_act = mysql_query("SELECT ac.descripcion
                                FROM actividad_grupo_empresa ac
                                WHERE ac.id_actividad=".$id_act."",$conn)  or die("Could not execute the select query.");
$resultado_no_act = mysql_fetch_assoc($consulta_no_act);
$no_act=$resultado_no_act['descripcion'];
  ...
  ventana.close();  //cerramos la ventana
			...
			<div>
				<ul class="breadcrumb">
                    ...
                    <li>
						<a href='actividades_grupo_integra.php?entreprod=<?php echo $id_act;?>'> Actividades </a>  <span class="divider">/</span>
					</li>
                    <li>
						<a href='tareas_grupo_integra.php?entreprod=<?php echo $id_act;?>'> Tareas </a>
					</li>
				</ul>
			</div>
			...
			<div class="row-fluid">
				<div class="box span12 " id="print">
						<div class="box-header well">
							<h2><i class="icon-calendar"></i> Tareas de la actividad: <?php echo "".$no_act;  ?></h2>
						</div>
						 ...
						 <div class="row-fluid">
						   <!--	 <div class="span12" style="padding:10px;"> </div>    -->
                             <?php
                              ...
                              $entregas ="SELECT t.descripcion,t.fecha_inicio,t.fecha_fin,t.porcentaje_completado,u.nombre,u.apellido,t.resultado_obtenido
                                          FROM tarea t ,usuario u
                                          WHERE t.actividad=".$id_act." and t.responsable=u.id_usuario";
                                    ...
                                    <div class="controls">
                                                  <?php    echo "     <select id=\"entreprod\" name=\"entreprod\" data-rel=\"chosen\">   ";
                                                                                //busca todas las emtregas de producto de la grupo empresa
                                                                                       $resultado = mysql_query($entregas);
                                                                                  while($row = mysql_fetch_array($resultado)) {
                                                                                            echo "<option value=\"".$row['ac.id_actividad']."\">".$row['descripcion']."</option>";
                                                                                  }
                                                                         echo "	</select>  ";
                                                  ?>
                                        	</div>
